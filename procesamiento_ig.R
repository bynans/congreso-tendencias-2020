#Procesamiento de base de INSTAGRAM
...
file_list <- list.files(path="base_ig/", full.names = T)
...
for (i in 1:length(file_list)){
  ...
  temp_data <- temp_data %>% select(taken_at,user, caption)
...
regex_user <- "'username': '\\s*(.*?)\\s*', 'full_name'"
regex_caption <- "'text': '\\s*(.*?)\\s*', 'is_private'"

#Preparo el dataset para el análisis  
dataset_modificado <- dataset %>% 
  mutate(fecha_publicacion = as_datetime(as.numeric(taken_at)),
         fecha_publicacion = date(fecha_publicacion),
         usuario = str_match(user, regex(regex_user))[,2],
         post = str_match(caption, regex(regex_caption))[,2]) %>% 
  filter(fecha_publicacion >= fecha_inicio, fecha_publicacion <= fecha_fin) %>%
  select(fecha_publicacion, usuario, post)


#Posteos sin caption
post_na <- dataset_modificado[is.na(dataset_modificado$post),]
...
librerias_editoriales <- unique(tabla_mes_anio$usuario)
...
dataset_marcado <- dataset_modificado %>% 
  mutate(a_domicilio =ifelse(str_detect(post, regex(regex_domicilio, ignore_case = TRUE)), 1, 0),
         online = ifelse(str_detect(post, regex(regex_online, ignore_case = TRUE)), 1, 0),
         envio=ifelse(str_detect(post, regex(regex_envio, ignore_case = TRUE)), 1, 0),
         descuento=ifelse(str_detect(post, regex(regex_descuento, ignore_case = TRUE)), 1, 0),
         ebooks=ifelse(str_detect(post, regex(regex_ebook, ignore_case = TRUE)), 1, 0),
         en_casa=ifelse(str_detect(post, regex(regex_encasa, ignore_case = TRUE)), 1, 0) ) %>% 
  filter(!is.na(post))
